...
data.map(({ submission_link: url, id }) => {
  describe("Test", function () {
    ...
    function CheckBoard(data, score, index, id) {
      cy.get(`#${id}>div>div`).eq(index).contains(data.name);
      cy.get(`#${id}>div>div`).eq(index).contains(data.desc);
      cy.get(`#${id}>div>div`).eq(index).contains(data.meetType);
    ...
    if (url && url.length) {
      ...
      it("Check the board after changing the week", () => {
        cy.get("#Week-1>div>div").eq(0).children("select").select("Week-2");
        CheckBoard(Data[0], 0, 0, "Week-2");
        cy.get("#Week-1>div").children().should("have.length", 0);
      ...
      it(`Chage the week again after month changing`, () => {
        cy.get("#Week-1>div>div").eq(0).children("select").select("Week-3");
      ...
      it("Check the Deleting Functionality", () => {
        cy.visit(`${url}/calender.html`).then(() => {
          localStorage.setItem("meets", JSON.stringify(Data));
        });
        cy.get("#Week-1 > div > div")
          .eq(0)
          .children("button")
          .click()
          .then(() => {
            expect(JSON.parse(localStorage.getItem("meets")).length).eq(5);
          });
        cy.get("#Week-1 > div > div").should("have.length", 0);
        CheckBoard(Data[1], 0, 0, Data[1].week);
        CheckBoard(Data[2], 0, 1, Data[2].week);
